...
function getColleState(now, startingTime) {
    if (now - startingTime >= 1000 * 60 * 60)
        return COLLE_STATE.done;
    if (startingTime - now <= 1000 * 60 * 60)
        return COLLE_STATE.soon;
...
function updateRelativeTimes() {
    ...
    for (var i = 0; i < $spans.length; i++) {
        ...
        if (!$colle.classList.contains(newStateClass)) {
            for (var j = 0; j < COLLE_STATE_CLASSES.length; j++)
                $colle.classList.remove(COLLE_STATE_CLASSES[j]);
    ...
    for (var id in weeks) {
        var week = weeks[id];
        if (week.allDone)
            week.$week.parentElement.removeChild(week.$week);
    }
}
...
function laterInit() {
    ...
    $query.addEventListener("input", function() {
        if (hasFailed || $query.value === query)
            return;

        // Remember the name for when the page is opened again.
        if (window.localStorage)
            localStorage.setItem(LOCAL_STORAGE_KEYS.query, $query.value);
...
function scheduleCacheDocumentHtml() {
    var TIMEOUT = 400;
    if ("requestIdleCallback" in window) {
        if (scheduleCacheTimerHandle !== null)
            cancelIdleCallback(scheduleCacheTimerHandle);
