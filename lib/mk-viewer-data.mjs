...
export default function makeViewerData(classes) {
    ...
    for (const classData of classes) {
        ...
        for (const week of classData.programWeeks) {
        ...
        if (Object.keys(subjectUrls).length > 0)
            classOut.push(subjectUrls);
        ...
        for (let i = 0; i < classData.studentGroups.length; i++) {
            const group = classData.studentGroups[i];
            ...
            for (let j = 0; j < group.students.length; j++) {
                ...
                if (group.programOverrides !== undefined &&
                    group.programOverrides[j] !== undefined) {
                    studentOut.push(group.programOverrides[j]
                        .map(delta => [
                            delta.week,
